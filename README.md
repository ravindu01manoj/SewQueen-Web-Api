# SewQueen - Typescript/Javascript WhatsApp Web API ( Cloned Version Of adiwajshing/baileys ) :)
 
 SewQueen does not require Selenium or any other browser to be interface with WhatsApp Web, it does so directly using a **WebSocket**. Not running Selenium or Chromimum saves you like **half a gig** of ram :/ 
 ...
 SewQueen is type-safe, extensible and simple to use. If you require more functionality than provided, it'll super easy for you to write an extension. More on this [here](#writing-custom-functionality).
 ...
 **Read the docs [here](https://adiwajshing.github.io/SewQueen)**
...
1. ``` cd path/to/SewQueen ```
2. ``` npm install ```
3. ``` npm run example ```

## Install
Create and cd to your NPM project directory and then in terminal, write: 
1. stable: `npm install @ravindu01manoj/web-sew`
2. stabl-ish w quicker fixes & latest features: `npm install github:ravindu01manoj/web-sew` 

Do note, the library will likely vary if you're using the NPM package, read that [here](https://www.npmjs.com/package/@ravindu01manoj/web-sew)

Then import in your code using:
``` ts 
import { WAConnection } from '@ravindu01manoj/web-sew'
```

## Unit Tests
SewQueen also comes with a unit test suite. Simply cd into the SewQueen directory & run `npm test`.
...
``` ts
import { WAConnection, ProxyAgent } from '@ravindu01manoj/web-sew'
...
SewQueen now uses the EventEmitter syntax for events. 
They're all nicely typed up, so you shouldn't have any issues with an Intellisense editor like VS Code.

Also, these events are fired regardless of whether they are initiated by the SewQueen client or are relayed from your phone.
...
``` ts
import { MessageType, MessageOptions, Mimetype } from '@ravindu01manoj/web-sew'
...
- When specifying a media url, SewQueen never loads the entire buffer into memory, it even encrypts the media as a readable stream.
...
- **MessageOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from '@ravindu01manoj/web-sew'
                                */
...
``` ts
import { Presence } from '@ravindu01manoj/web-sew'
...
``` ts
import { MessageType } from '@ravindu01manoj/web-sew'
...
- To change group settings
    ``` ts
    import { GroupSettingChange } from '@ravindu01manoj/web-sew'
...
SewQueen is written, keeping in mind, that you may require other custom functionality. Hence, instead of having to fork the project & re-write the internals, you can simply write extensions in your own code.
